...
<body>
    <!-- 페이징 처리를 위한 정보 전달 -->
    <form action="http://localhost:52525/BaCode/searchWhat" method="get">
    <input type="hidden" name="search_name" value="${searchWhat }"><br/>
    </form>
	<!-- ${boardList } -->
    <h1>키보드 관련 상품 페이지</h1>
    ...
    <nav aria-label="Page navigation example">
          ...
          <c:if test="${buttons.startPage ne 1}">
		    <li class="page-item"><a class="page-link" href="http://localhost:52525/BaCode/searchWhat?pageNum=${buttons.startPage-1 }">Previous</a></li>
		  </c:if>
    <c:forEach var="pageNum" begin="${buttons.startPage }" end="${buttons.endPage }"> 
		    <li class="page-item ${(buttons.currentPage eq pageNum) ? 'active' : ' '}" aria-current="page"><a class="page-link" href="http://localhost:52525/BaCode/searchWhat?pageNum=${pageNum}">${pageNum }</a></li>
    </c:forEach> 
          <c:if test="${buttons.endPage ne buttons.totalPages}">
		    <li class="page-item"><a class="page-link" href="http://localhost:52525/BaCode/searchWhat?pageNum=${buttons.endPage+1 }">Next</a></li>
		  </c:if>
		  </ul>
	</nav>
