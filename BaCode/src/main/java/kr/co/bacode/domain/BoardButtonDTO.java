	...
	// 굳이 차이를 두자면 VO와 DB에서 가져온 자료를 그대로 전달할 때,
	// DTO는 DB에서 꺼내온 데이터가 가공되어 전달할 때 쓰는 것을 지칭합니다.
	...
	public class BoardButtonDTO {
		...
		public BoardButtonDTO(int boardCount, int currentPage) {
			...
			// boardCount, currentPage를 맴버변수에 저장해주세요.
			...
			if(this.boardCount == 0) {
				...
				/* if(this.currentPage % 10 == 0) {
					// 10 배수 페이지는 자신이 속한 그룹보다 페이지 식별숫자가 1 크게 나옴
					navNum = (this.currentPage - 1) / 10;
				} else {
